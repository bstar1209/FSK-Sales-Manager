		...
		var g = {
			...
			dragEnd: function () {
				if (this.colresize) {
					...
					/* HocN add */
					if (p.onResizeComplete) {
                        p.onResizeComplete(t.id,$('th:visible div:eq(' + n + ')', this.hDiv).parent().attr('abbr'),nw);
                    }
				} else if (this.vresize) {
					this.vresize = false;
				} else if (this.colCopy) {
					/* HocN add */
					var colname = $('th:eq(' + this.dcoln + ')', this.hDiv).attr("abbr");
					$(this.colCopy).remove();
					if (this.dcolt !== null) {
						...
						if (p.onDragCol) {
							...
							/* HocN add */
							p.onDragCol(this.dcoln, this.dcolt,t.id,colname);
						}
					}
					this.dcol = null;
					this.hset = null;
